...
class StrandSet(QObject):
    ...
    def getLegacyArray(self):
        ...
        if self.isDrawn5to3():
            for strand in self._strandList:
                ...
                if s5p != None and s5p.part() is self.part():
                ...
                if s3p != None and s3p.part() is self.part():
    ...
    class CreateStrandCommand(QUndoCommand):
        ...
        def __init__(self, strandSet, baseIdxLow, baseIdxHigh, strandSetIdx,
            ...
            if color is None:
                colorList = (styles.stapColors if strandSet.isStaple()
    ...
    class RemoveStrandCommand(QUndoCommand):
        ...
        def __init__(self, strandSet, strand, strandSetIdx, solo=True):
            ...
            if olg.isLoop() or self._oldStrand3p == None:
                ...
                # Removing a strand can split either a staple or scaffold
                # oligo.  Both products retain the original oligo color.
                olg3p.setColor(olg.color())
                olg3p.refreshLength()
        # end def
        ...
        def redo(self):
            ...
            strand = self._strand
            strandSet = self._strandSet
            if strandSet.isScaffold():
                self._removedScaffoldOnly = \
                    strandSet.part().takeScaffoldOnlyIntervals(
                        strandSet.virtualHelix().coord(),
        ...
        def undo(self):
            ...
            if strandSet.isScaffold() and self._removedScaffoldOnly:
                strandSet.part().addScaffoldOnlyRegions({
                    strandSet.virtualHelix().coord():
                        self._removedScaffoldOnly})
            # strandSet._strandList.insert(self._sSetIdx, strand)
            strand5p = self._oldStrand5p
