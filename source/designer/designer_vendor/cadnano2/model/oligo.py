...
class Oligo(QObject):
    ...
    def locString(self):
        vhNum = self._strand5p.strandSet().virtualHelix().number()
        idx = self._strand5p.idx5Prime()
        return self._formatEndpoint(self._strand5p, idx)
    # end def
    ...
    def latticeCategory(self):
        """Return honeycomb, square, or hybrid for the physical oligo."""
        lattices = set()
        if self._strand5p is None:
            return 'honeycomb' if self._part._step == 21 else 'square'
        for strand in self._strand5p.generator3pStrand():
            lattices.add(strand.part()._step)
        if len(lattices) > 1:
            return 'hybrid'
        return 'honeycomb' if 21 in lattices else 'square'

    def isHybrid(self):
        return self.latticeCategory() == 'hybrid'

    def _formatEndpoint(self, strand, index, includeLattice=True):
        prefix = ''
        document = strand.document()
        if includeLattice and document is not None and document.isHybrid():
    ...
    def actualLength(self):
        """Return the current physical nucleotide count.

        ``_length`` is maintained incrementally by legacy editing commands
        and can become stale after an indel-rich design is reconstructed from
        JSON.  Deriving the value from every strand keeps UI length displays
        and exports correct for insertions and skips/deletions as well as for
        crossover-spanning oligos.
        """
        if self._strand5p is None:
            return 0
        return sum(strand.totalLength()
                   for strand in self._strand5p.generator3pStrand())
    # end def
    ...
    def sequenceExport(self, includeLattice=True):
        record = self.sequenceRecord(includeLattice=includeLattice)
        return "%s,%s,%s,%s,%s\n" % record
    # end def

    def sequenceRecord(self, includeLattice=True):
        """Return the common spreadsheet fields for this oligo."""
        start, end = self.sequenceEndpoints(includeLattice=includeLattice)
        seq = ''
        for strand in self.strand5p().generator3pStrand():
            seq = seq + Strand.sequence(strand, forExport=True)
        return (start, end, seq, len(seq), self._color)
    # end def

    def sequenceEndpoints(self, includeLattice=True):
        """Return formatted 5' and 3' endpoints for a non-circular oligo."""
        if self.isLoop() or self.strand5p() is None:
            raise Exception("Circular oligos do not have sequence endpoints")
        strand5p = self.strand5p()
        start = self._formatEndpoint(
            strand5p, strand5p.idx5Prime(), includeLattice)
        end = None
        for strand in strand5p.generator3pStrand():
            if strand.connection3p() is None:
                end = self._formatEndpoint(
                    strand, strand.idx3Prime(), includeLattice)
        if end is None:
            raise Exception("Could not determine the 3' endpoint")
        return start, end
    # end def
    ...
    def shouldHighlight(self):
        ...
        if self._strand5p.isScaffold():
            return False
        actualLength = self.actualLength()
        if actualLength < 18:
            return True
        if actualLength > 60:
            return True
