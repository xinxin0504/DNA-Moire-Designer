    ...
    class ResizeCommand(QUndoCommand):
        ...
        def redo(self):
            ...
            if strandSet.isScaffold():
                removedRanges = []
                if nI[0] > self.oldIndices[0]:
                    removedRanges.append((self.oldIndices[0], nI[0] - 1))
                if nI[1] < self.oldIndices[1]:
                    removedRanges.append((nI[1] + 1,
                                          self.oldIndices[1]))
                self._removedScaffoldOnly = []
                for low, high in removedRanges:
                    self._removedScaffoldOnly.extend(
                        part.takeScaffoldOnlyIntervals(
                            strandSet.virtualHelix().coord(), low, high))
