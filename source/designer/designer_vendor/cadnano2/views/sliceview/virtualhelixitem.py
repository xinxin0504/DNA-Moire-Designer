...
class VirtualHelixItem(QGraphicsEllipseItem):
    ...
    def createArrow(self):
        rad = self._radius
        scaffoldPen = QPen()
        scaffoldPen.setWidth(3)
        scaffoldColor = QColor(Qt.GlobalColor.blue)
        scaffoldColor.setAlphaF(0.25)
        scaffoldPen.setBrush(scaffoldColor)
        staplePen = QPen()
        staplePen.setWidth(3)
        stapleColor = QColor(Qt.GlobalColor.red)
        stapleColor.setAlphaF(0.25)
        staplePen.setBrush(stapleColor)
        if self._virtualHelix.isEvenParity():
            arrow = QGraphicsLineItem(rad, rad, 2*rad, rad, self)
            stapleArrow = QGraphicsLineItem(rad, rad, 2*rad, rad, self)
        else:
            arrow = QGraphicsLineItem(0, rad, rad, rad, self)
            stapleArrow = QGraphicsLineItem(0, rad, rad, rad, self)
        arrow.setTransformOriginPoint(rad, rad)
        stapleArrow.setTransformOriginPoint(rad, rad)
        arrow.setZValue(400)
        stapleArrow.setZValue(400)
        arrow.setPen(scaffoldPen)
        stapleArrow.setPen(staplePen)
        self.arrow = arrow
        self.stapleArrow = stapleArrow
        self.arrow.hide()
        self.stapleArrow.hide()
    # end def
    ...
    def updateArrow(self, idx):
        ...
        # The complementary staple backbone is on the opposite side of the
        # duplex at the same active base.
        self.stapleArrow.setRotation(angle + part._twistOffset + 180)
    # end def
