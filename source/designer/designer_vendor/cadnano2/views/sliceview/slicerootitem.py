...
class SliceRootItem(QGraphicsRectItem):
    ...
    from the model.
    """
    def __init__(self, rect, parent, window, document, partFilter=None):
        super(SliceRootItem, self).__init__(rect, parent)
        self._window = window
        self._document = document
        self._controller = ViewRootController(self, document)
        self._instanceItems = {}
        self._partFilter = partFilter
    ...
    def partAddedSlot(self, sender, modelPart):
        ...
        """
        if self._partFilter is not None and \
                not self._partFilter(modelPart):
            return
