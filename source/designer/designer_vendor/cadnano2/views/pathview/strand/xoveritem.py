    ...
    def _updateLabel(self, isLeft):
        ...
        if self._idx != None:
            if lbl == None:
                ...
                partnerText = str(num)
                if self._partnerVirtualHelix.part() is not \
                        self._vhi.virtualHelix().part():
                    prefix = ('H:' if self._partnerVirtualHelix.part()._step
                              == 21 else 'S:')
                    partnerText = prefix + partnerText
                tBR = _fm.tightBoundingRect(partnerText)
                ...
                # create text item
                lbl = QGraphicsSimpleTextItem(partnerText, self)
                lbl.setPos(labelX, labelY)
                lbl.setBrush(_enabbrush)
                lbl.setFont(_toHelixNumFont)
                self._label = lbl
            # end if
            partner = self._partnerVirtualHelix
            partnerText = str(partner.number())
            if partner.part() is not self._vhi.virtualHelix().part():
                partnerText = ('H:' if partner.part()._step == 21
                               else 'S:') + partnerText
            lbl.setText(partnerText)
        # end if
    # end def
...
class XoverItem(QGraphicsPathItem):
    ...
    def showIt(self):
        self.show()
        if self._node5:
            connected = (self._strand5p.connection3p()
                         if self._strand5p is not None else None)
            if connected is None or \
                    connected.part() is self._strand5p.part():
                self._node5.show()
            else:
                self._node5.hide()
        if self._node3:
    ...
    def refreshXover(self):
        ...
        if strand5p:
            ...
            if strand3p is not None and \
                    strand3p.part() is not strand5p.part():
                self.update(self._strand5p)
            elif strand3p != None and node3:
    ...
    def _updateHybridPath(self, strand5p):
        """Keep a local hit target; the shared overlay draws the artwork."""
        node5 = self._node5
        parent = self.partItem()
        point = self._virtualHelixItem.mapToItem(parent, *node5.point())
        center = point + QPointF(.5, .5) * _baseWidth
        painterpath = QPainterPath()
        painterpath.moveTo(center)
        self.setPath(painterpath)
        self._clickArea.setRect(QRectF(center, center).adjusted(
            -_baseWidth / 2, -_baseWidth / 2,
            _baseWidth / 2, _baseWidth / 2))
        node5.hide()
        self.setPen(QPen(Qt.PenStyle.NoPen))
    ...
    def _updateColor(self, strand):
        ...
        if strand.isScaffold():
            penWidth = styles.PATH_SCAFFOLD_STROKE_WIDTH
        elif oligo.shouldHighlight():
        ...
        if connected is not None and connected.part() is not strand.part():
            # A transparent, base-sized hit area remains in this panel.  The
            # visible full crossover is painted by HybridConnectionOverlay.
            self.setPen(QPen(Qt.PenStyle.NoPen))
            overlay = getattr(self._strandItem.window(),
                              'hybridConnectionOverlay', None)
    ...
    def mousePressEvent(self, event):
        ...
        """
        activeToolName = str(self.activeTool())
        strand5p = self._strand5p
        strand3p = (strand5p.connection3p()
                    if strand5p is not None else None)
        isHybridConnection = (
            strand3p is not None and
            strand3p.part() is not strand5p.part())
        # The shared Hybrid curve is drawn by a transparent widget so that
        # normal panel interaction remains available.  Its base-anchored
        # local hit target lives here: clicking it with Select or the
        # Cross-lattice tool removes the connection immediately, exactly as
        # an ordinary crossover can be removed from its hit target.
        if isHybridConnection and activeToolName in (
                                                "selectTool", "hybridTool"):
            event.setAccepted(True)
            strand5p.document().removeHybridConnection(strand5p, '3p')
            return
        if activeToolName == "selectTool":
