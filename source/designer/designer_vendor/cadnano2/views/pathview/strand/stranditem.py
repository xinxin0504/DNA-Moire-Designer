...
class StrandItem(QGraphicsLineItem):
    ...
    def strandRemovedSlot(self, strand):
        ...
        for marker in self._scaffoldOnlyItems:
            scene.removeItem(marker)
        self._scaffoldOnlyItems = None
    ...
    def _updateHighlight(self, color):
        ...
        if self._modelStrand.isScaffold():
        ...
        self._updateScaffoldOnlyItems(color)
    # end def

    def _updateScaffoldOnlyItems(self, color=None):
        """Draw auto-added, unpaired scaffold intervals as normal-width dashes."""
        if self._scaffoldOnlyItems is None:
            return
        scene = self.scene()
        for marker in self._scaffoldOnlyItems:
        ...
        self._scaffoldOnlyItems = []
        strand = self._modelStrand
        if strand is None or not strand.isScaffold():
            return
        low, high = strand.idxs()
        intervals = strand.part().scaffoldOnlyIntervals(
                                                strand.virtualHelix())
        if color is None:
            color = QColor(strand.oligo().color())
        pen = QPen(color, styles.PATH_SCAFFOLD_STROKE_WIDTH)
        pen.setStyle(Qt.PenStyle.DashLine)
        pen.setCapStyle(Qt.PenCapStyle.FlatCap)
        vhi = self._virtualHelixItem
        for excludeLow, excludeHigh in intervals:
            markLow = max(low, excludeLow)
            markHigh = min(high, excludeHigh)
            if markLow > markHigh:
                continue
            leftX, upperY = vhi.upperLeftCornerOfBase(markLow, strand)
            rightX, unusedY = vhi.upperLeftCornerOfBase(markHigh, strand)
            y = upperY + _baseWidth / 2.0
            marker = QGraphicsLineItem(
                leftX + _baseWidth / 2.0, y,
                rightX + _baseWidth / 2.0, y, self)
            marker.setPen(pen)
            marker.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
            self._scaffoldOnlyItems.append(marker)
    ...
    def mousePressEvent(self, event):
        ...
        """
        part = self._modelStrand.part()
        part.document().setSelectedPart(part)
        self.window().controller.setActivePart(part)
    ...
    def hoverMoveEvent(self, event):
        ...
        idx = int(floor((event.pos().x()) / _baseWidth))
        oligoLength = self._modelStrand.oligo().actualLength()
    ...
    def paint(self, painter, option, widget):
        ...
        excluded = []
        vhi = self._virtualHelixItem
        for intervalLow, intervalHigh in strand.part().scaffoldOnlyIntervals(
                                                    strand.virtualHelix()):
            markLow = max(low, intervalLow)
            markHigh = min(high, intervalHigh)
            if markLow > markHigh:
                continue
            leftX, unusedY = vhi.upperLeftCornerOfBase(markLow, strand)
            rightX, unusedY = vhi.upperLeftCornerOfBase(markHigh, strand)
            excluded.append((leftX + _baseWidth / 2.0,
                             rightX + _baseWidth / 2.0))
        if not excluded:
            painter.drawLine(self.line())
            return
        ...
        y = line.y1()
        cursor = startX
        for excludeStart, excludeEnd in sorted(excluded):
            excludeStart = max(startX, excludeStart)
            excludeEnd = min(endX, excludeEnd)
            if excludeEnd < cursor:
                continue
            if excludeStart > cursor:
                painter.drawLine(QLineF(cursor, y, excludeStart, y))
            cursor = max(cursor, excludeEnd)
        if cursor < endX:
            painter.drawLine(QLineF(cursor, y, endX, y))
    # end def
