...
class EndpointItem(QGraphicsPathItem):
    ...
    def __init__(self, strandItem, captype, isDrawn5to3):
        ...
        self.setPen(QPen())
        self._scaffoldStartMarker = QGraphicsEllipseItem(self)
        self._scaffoldStartMarker.setPen(QPen(QColor('#222222'), 1.2))
        self._scaffoldStartMarker.setBrush(QBrush(QColor('#ffffff')))
        self._scaffoldStartMarker.setAcceptedMouseButtons(
                                            Qt.MouseButton.NoButton)
        self._scaffoldStartMarker.setZValue(1)
        self._updateScaffoldStartMarker(isDrawn5to3)
    ...
    def _updateScaffoldStartMarker(self, isDrawn5to3):
        """Show a circle inside only the scaffold's physical 5' square."""
        strand = self._strandItem._modelStrand
        capIs5p = (self._capType == 'dual' or
                   (self._capType == 'low' and isDrawn5to3) or
                   (self._capType == 'high' and not isDrawn5to3))
        if not strand.isScaffold() or not capIs5p:
            self._scaffoldStartMarker.hide()
            return

        if self._capType == 'dual':
            centerX = (0.25 if isDrawn5to3 else 0.75) * _baseWidth
        else:
            centerX = (0.625 if isDrawn5to3 else 0.375) * _baseWidth
        centerY = 0.5 * _baseWidth
        radius = 0.18 * _baseWidth
        self._scaffoldStartMarker.setRect(centerX - radius,
                                          centerY - radius,
                                          2 * radius, 2 * radius)
        self._scaffoldStartMarker.show()
    # end def
    ...
    def mousePressEvent(self, event):
        ...
        """
        part = self._strandItem._modelStrand.part()
    ...
    def hoverMoveEvent(self, event):
        ...
        oligoLength = self._strandItem._modelStrand.oligo().actualLength()
    ...
    def hybridToolMousePress(self, modifiers, event, idx):
        """Select one physical endpoint for a cross-lattice connection."""
        strand = self._strandItem._modelStrand
        if idx == strand.idx5Prime() and idx != strand.idx3Prime():
            end = '5p'
        elif idx == strand.idx3Prime() and idx != strand.idx5Prime():
            end = '3p'
        else:
            self.partItem().updateStatusBar(
                "Single-base strands are ambiguous; extend the strand before "
                "creating a Hybrid connection.")
            return
        self._activeTool().endpointClicked(strand, end)
