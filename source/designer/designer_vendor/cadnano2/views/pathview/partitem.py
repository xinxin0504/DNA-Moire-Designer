...
class PartItem(QGraphicsRectItem):
    ...
    def _setVirtualHelixItemList(self, newList, zoomToFit=True):
        ...
        # Keep the model's Path-view order current for newly created designs,
        # not only after an explicit drag reorder or a save/reload cycle.
        # AutoCS scaffold uses this order to decide which helix rows are
        # adjacent on the right-hand panel.
        if self._modelPart is not None:
            self._modelPart.setImportedVHelixOrder(
                self.getOrderedVirtualHelixList(), emitSignal=False)
        if zoomToFit:
            self.scene().views()[0].zoomToFit()
    # end def
    ...
    def setPreXoverItemsVisible(self, virtualHelixItem):
        ...
        nativeXovers = part.potentialCrossoverList(vh, idx)
        supplementalXovers = part.supplementalStapleCrossoverList(vh, idx)
        for potentialXovers, isSupplemental in ((nativeXovers, False),
                                                 (supplementalXovers, True)):
            for neighbor, index, strandType, isLowIdx in potentialXovers:
                # create one half
                neighborVHI = self.itemForVirtualHelix(neighbor)
                pxi = PreXoverItem(vhi, neighborVHI, index, strandType,
                                   isLowIdx, isSupplemental)
                # add to list
                self._preXoverItems.append(pxi)
                # create the complement
                pxi = PreXoverItem(neighborVHI, vhi, index, strandType,
                                   isLowIdx, isSupplemental)
                # add to list
                self._preXoverItems.append(pxi)
        # end for
