...
class Mixin:
    ...
    def route_staples(self):
        ...
        for edge in self.pathway_edges:
            ...
            requested_xovers = math.gcd(ring2.numxovers, ring1.numxovers) - 1
            # calibrate and get first xover position
            refxoverpair = self.calibrate_ring(edge)
            # get all other xover positions
            # First collect every currently legal site.  Passing the desired
            # count into get_xovers() makes its spacing filter reject the
            # *entire edge* when fewer sites than requested survive.  That
            # created completely unbridged neighbouring rings.  Distribution
            # can safely choose the requested count after candidates exist.
            xoverset = crossover.get_xovers(ring1, ring2, 'stap',
                                       threshold=edge.thresh, numxovers=0)
            # The published legacy router assumes every requested crossover
            # survives the geometry/spacing filter.  Real parameterised rings
            # can provide fewer legal sites; use all legal sites instead of
            # letting distribute() index past the candidate list.
            numxovers = min(requested_xovers, len(xoverset))
            if numxovers <= 0:
                continue
    ...
    def route_asym_staples(self):
        ...
        # The upstream source left this call commented out, which makes the
        # simulated-annealing optimizer start from an entirely empty graph.
        # Its gap-filling loop then keeps expanding impossible sites.  Build
        # the complete geometry- and spacing-legal initial set first; this is
        # deterministic and provides a valid fast route without annealing.
        self.xover_set.init_basic(
            bondlen=config.VALIDXOVERTHRESHBP,
            xoverspacing1=config.VALIDXOVERSPACING_SAME,
            xoverspacing2=config.VALIDXOVERSPACING_ADJ)
        self.xover_set.thin_to_density(config.XOVER_FACTOR)
        self.xover_set.fill_empty_edges(
            bondlen=config.VALIDXOVERTHRESHBP,
            xoverspacing1=config.VALIDXOVERSPACING_SAME,
            xoverspacing2=config.VALIDXOVERSPACING_ADJ)
