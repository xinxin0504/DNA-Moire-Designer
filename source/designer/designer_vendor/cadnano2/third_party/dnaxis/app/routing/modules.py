...
class Plane(object):
    ...
    def add_circle(self, idx, bp, dir_bit, **kwargs):
        ...
        :return:
        """
        target_radius = kwargs.pop('radius', None)
        ...
        shape_obj = makeshape.CircleOnPlane(
            bp, self.origin, self.basis, radius=target_radius)
        dna_obj = RingModule(shape_obj.num_points, turns, dir_bit, self.origin, self.bz, **kwargs)
        dna_obj.applyshape(shape_obj, idx)
        # Curved Design deliberately decouples the equal-length lattice
        # parent from its target geometric radius.  Native DNAxiS callers that
        # omit radius retain the original bp-derived geometry.
        dna_obj.radius = shape_obj.radius
