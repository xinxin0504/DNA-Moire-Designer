...
class CrossoverSet:
    ...
    def __init__(self, dnaorigami, **kwargs):
        ...
        self.active_xovers = []
        self._nucl_xover_index = {}
        for xovers in self.positions.values():
            for xover in xovers:
                self._index_xover(xover)

    def _index_xover(self, xover):
        for nucleotide in xover.get_nucls():
            self._nucl_xover_index[id(nucleotide)] = xover

    def _unindex_xover(self, xover):
        for nucleotide in xover.get_nucls():
            key = id(nucleotide)
            if self._nucl_xover_index.get(key) is xover:
                del self._nucl_xover_index[key]

    def _rebuild_nucl_index(self):
        self._nucl_xover_index = {}
        for xovers in self.positions.values():
            for xover in xovers:
                self._index_xover(xover)
    ...
    def init_basic(self, bondlen=config.VALIDXOVERTHRESHBP,
        ...
        for edge in self.edges:
            ...
            for xover in proposed_xovers:
                self._index_xover(xover)

    def fill_empty_edges(self,
                         bondlen=config.VALIDXOVERTHRESHBP,
                         xoverspacing1=config.VALIDXOVERSPACING_SAME,
                         xoverspacing2=config.VALIDXOVERSPACING_ADJ):
        """Repair only neighbour pairs left empty by the legal initial set."""
        for edge in self.edges:
            if self.positions[edge]:
                continue
            module1 = edge.directed['from']
            module2 = edge.directed['to']
            region = (module1.helix.stap if module1.bp >= module2.bp
                      else module2.helix.stap)
            self.ins_xover(region, edge, bondlen,
                           xoverspacing1, xoverspacing2)

    def thin_to_density(self, crossover_factor=4):
        """Keep an even, synthesis-friendly subset on each neighbour edge."""
        for edge in self.edges:
            candidates = self.positions[edge]
            if not candidates:
                continue
            module1 = edge.directed['from']
            module2 = edge.directed['to']
            desired = max(1, int(round(
                min(module1.bp, module2.bp) /
                (10.5 * float(crossover_factor)))))
            desired = min(desired, len(candidates))
            if desired < len(candidates):
                self.positions[edge] = crossover.distribute(
                    candidates, desired, 'stap')
        self._rebuild_nucl_index()
    ...
    def ins_xover(self, region, ref_edge,
        ...
        bondlen_limit = bondlen + getattr(config, 'THRESH_ADD', 1.0)
        ...
        while True:
            ...
            if not xplist:
                if bondlen >= bondlen_limit:
                    log.log_warning(
                        "No geometry-legal crossover could repair edge {} "
                        "within bond threshold {:.1f}.".format(
                            ref_edge, bondlen_limit))
                    return False
    ...
    def nucl_top_xover(self, nucl, detect_scaf=False):
        ...
        """
        indexed = self._nucl_xover_index.get(id(nucl))
