...
def distribute(xoverpairs, numxovers, strandtype):
    ...
    if numxovers == 1:
        # ``ceil(len / 2)`` is a one-based midpoint applied to a zero-based
        # Python list.  It fails for a single legal crossover candidate and
        # biases every odd-sized candidate set to the right.  Curved surfaces
        # legitimately produce one-candidate edge pairs near caps, so select
        # the actual zero-based midpoint instead.
        s_arr = ([arr[len(arr) // 2]], 0.0, True)
    else:
...
def get_xovers(module1, module2, strand, **kwargs):
    ...
    while True:
        try:
            ...
            xplist = filterdist2feat(
                xplist, numxovers=num,
                spacing_init=kwargs.get(
                    "spacing_init", config.VALIDXOVERSPACING_SAME))
