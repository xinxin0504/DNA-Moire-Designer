...
def import_legacy_dict(document, obj, latticeType=LatticeType.Honeycomb):
    ...
    # Preserve Moiré Designer extension fields verbatim.  Topology,
    # dimensions, colours and applied sequences are regenerated from the
    # model when saved; all other fields are round-tripped unchanged.
    generated_keys = {
        'name', 'vstrands', 'num_bases', 'lattice',
        'scaffold_colors', 'scaffold_sequences'}
    document._moireJsonExtensions = deepcopy({
        key: value for key, value in obj.items()
        if key not in generated_keys})

    lattice_name = str(obj.get('lattice', '')).strip().lower()
    lattice_is_explicit = lattice_name in ('square', 'honeycomb')
    if lattice_name == 'square':
        latticeType = LatticeType.Square
    elif lattice_name == 'honeycomb':
        latticeType = LatticeType.Honeycomb

    if cadnano.app().isGui() and not lattice_is_explicit:
    ...
    # Restore top-level scaffold colours and applied sequences only after the
    # complete strand/crossover topology exists.  Invalid or stale records are
    # ignored so ordinary and externally edited caDNAno JSON remains loadable.
    for record in obj.get('scaffold_colors', []):
        ...
        strand = vh.scaffoldStrandSet().getStrand(startIdx) if vh else None
        if strand is None or strand.idx5Prime() != startIdx:
            continue
        if strand.oligo().strand5p() is not strand:
            continue
